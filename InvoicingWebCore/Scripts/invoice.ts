...
function AddItem() {
    ...
    if (window.window._rowId === undefined) window.window._rowId = 0;

    var productList : any = $("#viewBagProductList").val();
    var taxesList : any = $("#viewBagTaxTypes").val();
    var quantityUnits :any = $("#viewBagQuantityUnits").val();
    ...
    if (contractorId != "" && contractorId != null && contractorId != 0) {
        $.ajax({
            ...
            success: function (contractor) {
                $.each(contractor, function (k : string, v : string) {
                    if (k != null && k != "") {
                        if (v != null && v != "") {
                            $("#" + k).val(v);
                        }
...
function UpdateTotalPrice(id: number) {
